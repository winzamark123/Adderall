...
public struct LeaseManager: Sendable {
    ...
    @discardableResult
    public mutating func begin(_ command: LeaseCommand, now: Date = Date()) -> Lease {
    ...
    @discardableResult
    public mutating func heartbeat(_ command: LeaseCommand, now: Date = Date()) -> Lease {
        begin(command, now: now)
    }

    @discardableResult
    public mutating func end(provider: String, sessionID: String) -> Lease? {
        leasesByKey.removeValue(forKey: LeaseKey(provider: provider, sessionID: sessionID))
    }

    @discardableResult
    public mutating func expireLeases(now: Date = Date()) -> [Lease] {
