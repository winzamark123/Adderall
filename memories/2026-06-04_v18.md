...
The next integration pass is Codex. Treat Codex as planned, not implemented.
...
- Use `.build/debug/adderall uninstall claude` or `.build/debug/adderall uninstall pi` to remove provider integrations. The shared user LaunchAgent is removed only when no known integrations remain.
- Use `adderall status --json`, simulated `adderall hook claude` stdin payloads, and Pi `/reload` after `install pi` to verify XPC, lease, snapshot, expiry, and normal-awake behavior.
- After changing the generated Pi extension or Claude hook behavior, rerun `adderall install pi` or `adderall install claude` to update installed integration files.
...
- Manual test flow that simulates Claude hooks and Pi extension behavior.
- Hook/event mapping documented for Claude Code, Pi, and planned Codex support.
...
- Codex hook installer in the next pass, if the local hook format is stable enough.
...
```mermaid
flowchart TD
    A["Claude hooks / Pi extension / future Codex hooks"] --> B["adderall CLI"]
...
For Claude Code and future Codex support, that means hook configuration. For Pi, that means an Adderall-owned TypeScript extension. In all cases, the integration invokes the `adderall` CLI automatically on lifecycle events. The agent model should not need to know Adderall exists.
...
Codex is the next planned integration. Hooks are relevant but should be treated as less mature than Claude Code and Pi for now.
