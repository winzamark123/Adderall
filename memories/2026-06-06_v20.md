...
- Claude Code through `adderall install claude`, user-level Claude hooks, and `adderall hook claude`.
- Codex through `adderall install codex`, user-level Codex hooks in `~/.codex/hooks.json`, and `adderall hook codex`.
- Pi through `adderall install pi` and an Adderall-owned Pi extension in `~/.pi/agent/extensions/adderall/index.ts`.
...
- Use `.build/debug/adderall install codex` to copy the current CLI/controller binaries into user application support storage, bootstrap the user LaunchAgent, and install Codex hooks. Then restart Codex, run `/hooks`, and trust the Adderall hooks.
- Use `.build/debug/adderall install pi` to copy the current CLI/controller binaries into user application support storage, bootstrap the user LaunchAgent, and install the Adderall Pi extension.
- Use `.build/debug/adderall uninstall claude`, `.build/debug/adderall uninstall codex`, or `.build/debug/adderall uninstall pi` to remove provider integrations. The shared user LaunchAgent is removed only when no known integrations remain.
- Use `adderall status --json`, simulated `adderall hook claude` / `adderall hook codex` stdin payloads, Codex `/hooks`, and Pi `/reload` after `install pi` to verify XPC, lease, snapshot, expiry, and normal-awake behavior.
- After changing the generated Pi extension or Claude/Codex hook behavior, rerun `adderall install pi`, `adderall install claude`, or `adderall install codex` to update installed integration files.
...
- Pi extension installer through `adderall install pi`.
- Pi extension uninstaller through `adderall uninstall pi`.
- Codex hook entrypoint through `adderall hook codex`.
- Codex hook installer through `adderall install codex`.
- Codex hook uninstaller through `adderall uninstall codex`.
...
- Pi lifecycle events invoke `adderall begin`, `adderall heartbeat`, and `adderall end` through an Adderall-owned Pi extension.
- Codex hooks invoke `adderall hook codex`.
- `adderall hook codex` maps hook JSON to `begin` and `heartbeat` requests over XPC.
...
```mermaid
flowchart TD
    A["Claude hooks / Codex hooks / Pi extension"] --> B["adderall CLI"]
...
Adderall should install lifecycle integration configuration for supported agents.

For Claude Code and Codex, that means hook configuration. For Pi, that means an Adderall-owned TypeScript extension. In all cases, the integration invokes the `adderall` CLI automatically on lifecycle events. The agent model should not need to know Adderall exists.
...
```text
agent runtime event -> installed hook/extension -> adderall CLI -> XPC -> user-level controller
```
...
`adderall hook claude` reads Claude Code's hook JSON from stdin and maps lifecycle events to controller lease commands using Claude's `session_id` as the lease session. Runtime hook mode fails open: if parsing, XPC, or the controller fails, it logs and exits successfully so Claude Code is not blocked. When a normal `Stop` event ends the lease, the hook emits a user-visible `systemMessage`: `Released adderall (agent finished).`
...
That command installs the user LaunchAgent/controller and writes an Adderall-owned extension at:

```text
~/.pi/agent/extensions/adderall/index.ts
```

The extension listens to Pi lifecycle events and invokes the installed `adderall` CLI. It does not hold power assertions itself; it only bridges Pi activity into the controller-owned lease protocol.
...
- `agent_end`: end the Pi lease, clear the Pi status UI, and notify `Released adderall (agent finished).`
- `session_shutdown`: end the Pi lease and clear Pi status UI.

Users remove the integration with:

```text
adderall uninstall pi
```

While active, the Pi footer status shows `awake`. If the extension cannot reach Adderall during active work, it shows `unavailable` and warns without blocking Pi.
...
```text
adderall install codex
```

That command installs the user LaunchAgent/controller and writes Adderall-owned Codex hooks at:
...
`adderall hook codex` reads Codex's hook JSON from stdin and maps lifecycle events to controller lease commands using Codex's `session_id` as the lease session. Runtime hook mode fails open: if parsing, XPC, or the controller fails, it logs and exits successfully so Codex is not blocked.

Codex requires non-managed command hooks to be reviewed and trusted before they run. After installing, restart Codex, run `/hooks`, and trust the Adderall hooks.

Users remove the integration with:

```text
adderall uninstall codex
```
