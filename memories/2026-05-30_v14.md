...
- Use `Scripts/install-dev-launch-agent.sh` to build and bootstrap the controller as a user LaunchAgent during local development.
- Use `Scripts/smoke-test-dev-launch-agent.sh` to verify the LaunchAgent, XPC, lease, snapshot, restart, expiry, and normal-awake path.
- Use `Scripts/install-claude-hooks.sh` to install development Claude Code hooks that invoke `adderall hook claude`.
- Use `Scripts/uninstall-claude-hooks.sh` to remove those development Claude Code hooks.
- Use `Scripts/uninstall-dev-launch-agent.sh` to stop and remove the development LaunchAgent.
...
- Hook diagnostics.
- Manual hook simulation scripts or documentation.

Success criteria:

- Hooks invoke `adderall begin`, `heartbeat`, and `end`.
- Hook commands are fast and non-interactive.
- Hook failures are visible in diagnostics.
...
Claude Code has mature lifecycle hooks. Adderall installs command hooks that invoke:

```text
adderall hook claude
```

That command reads Claude Code's hook JSON from stdin and maps lifecycle events to controller lease commands using Claude's `session_id` as the lease session.
...
Adderall/
  ...
  Packaging/
    com.example.adderall.controller.plist
    com.example.adderall.helper.plist
  Scripts/
    install-claude-hooks.sh
    install-dev-launch-agent.sh
    smoke-test-dev-launch-agent.sh
    uninstall-claude-hooks.sh
    uninstall-dev-launch-agent.sh
  memories/
  AGENTS.md
  README.md
```
